function [vrtx, nbr_vrtx, adjmat_white] = adjmat_directed_flat(img, layer, Wt_exp, Wt_horz, Wt_dir) % refdirections
...
Wt_horizontal = ( Wt_horz * abs(dx) + abs(dy)) ./ (abs(dx) + abs(dy));

edge_weights = Wt_horizontal;
                  

% Wt = 2;
% edge_weights = abs(dx) + Wt * abs(dy);
